...
def make_model(nb_epochs=100, batch_size=128, lr=0.01, n_layers=1, n_hidden=14, rate_dropout=0.3, loss=risk_estimation):
    ...
    model_path = 'model_%s' % input_shape[0]
    ...
    wp.fit(train_set.images, train_set.labels, batch_size=batch_size,
           ...
           validation_data=(test_set.images, test_set.labels),
            # commenter ModelCheckpoint pour éviter l’erreur "options"

           #callbacks=[ModelCheckpoint(filepath=model_path + '.best.h5', save_best_only=True, mode='min')]
           )
    ...
    ########## Désactivé temporairement pour éviter erreur 'options' non supporté
    ### ca marche comme ca mais une fois l'entrainement terminé, plus aucune mémoire... un peu bete
    # wp.model.save(model_path + '.h5')
...
def make_separate_model(nb_epochs=100, batch_size=128, lr=0.01, n_layers=1, n_hidden=14, rate_dropout=0.3, input_shape=[30, 73]):
    ...
    for code, train_set in train_sets.items():
        ...
        wp.fit(train_set.images, train_set.labels, batch_size=batch_size,
               epochs=nb_epochs, shuffle=False, verbose=1,
               validation_data=(test_set.images, test_set.labels),
